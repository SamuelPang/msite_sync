  ...
  const ScoreEditor = () => {
    ...
    const parseJianpu = (jianpuStr) => {
      const jianpuMap = { '1': 'c', '2': 'd', '3': 'e', '4': 'f', '5': 'g', '6': 'a', '7': 'b', '0': null }; // 0 for rest
    ...
    while (targetDiv.firstChild) {
      ...
      if (notes.length > 0) {
        const vexNotes = notes.map(note =>
          new StaveNote({
            keys: [note.pitch === 'r' ? 'b/4' : note.pitch], // Use a dummy pitch for rests
    ...
    const exportScore = async (format) => {
      try {
        if (format === 'pdf') {
          ...
          console.log('Export payload:', { track_id: 0, start: 0, end: notes.length, format, instrument: selectedInstrument });

          const response = await axios.post(
            `${process.env.REACT_APP_BACKEND_URL}/api/scores/${currentScoreId}/export`,
            { track_id: 0, start: 0, end: notes.length, format, instrument: selectedInstrument }
          );
